...
const DescripcionEmpresa = ({ handler, goBack }: { handler: (key: string, value: any) => void, goBack: ()=>void }) => {
    ...
    <div className='flex flex-col items-center justify-center gap-10 pb-48'>
      <label className='flex justify-center flex-col pt-8 gap-2' htmlFor='descripcion_empresa'>Descripcion de la Empresa
      ...
      <div className='flex gap-4'>
        ...
        <button className='bg-blue-200 w-[5rem] h-8 rounded-lg' onClick={() => handler('descripcion_empresa', descripcion_empresa.current.value)}>Guardar</button>
      </div>
      </div>
      </div>
    </>
