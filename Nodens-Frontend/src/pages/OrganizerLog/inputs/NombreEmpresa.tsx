...
const NombreEmpresa = ({ handler, goBack }: { handler: (key: string, value: any) => void, goBack: ()=>void }) => {
    ...
    return (
    <>
      <div className='h-2/4 w-10/12 flex flex-col justify-center gap-8 px-2 text-slate-100 bg-slate-100 bg-opacity-10 shadow-xl rounded-xl'>
        ...
        <div className="flex w-3/5 gap-4">
          ...
          <button onClick={() => handler('nombre_empresa', "")} className='px-4 bg-slate-900 rounded-md text-orange-500 h-14'>No quiero añadir una empresa</button>
        </div>
      </div>
    </>
    )
