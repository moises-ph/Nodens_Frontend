...
const ModalProfile = ({open, profile, closeModal, canPostulate}:{open:boolean, profile: ProfileT | undefined, closeModal : any, canPostulate : boolean}) => {
	const variants = {
		open: {
			translateY: window.innerWidth > 768 ? '-87%' : '-70%'
		},
		closed : {
  ...
  return (
    <motion.div
		...
		variants={variants}
		className='md:h-[82%] h-2/3 overflow-y-auto md:left-[40%] p-4 pb-2 absolute md:fixed md:w-3/5 w-full bg-slate-100 rounded-lg  shadow-slate-300 shadow-inner rounded-t-xl'
