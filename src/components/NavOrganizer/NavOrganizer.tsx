...
const NavOrganizer:React.FC<NavBar> = ({inView, setShowNav}) =>  {
  ...
  return (
    <div className="z-[100] w-full h-screen fixed flex flex-col justify-start items-end pr-4 pt-4" onClick={()=>setShowNav(false)}>
			<nav className="bg-slate-700 h-4/5 w-5/6 flex flex-col items-start gap-5 pt-4 pl-4 rounded-xl sm:w-3/5 md:w-2/4 lg:w-1/4 transition-all nav1">
				...
				<NavLink className={({isActive})=>NavLinkStyles({isActive})} to="/offers"><BsFileEarmarkPost /> Tus Ofertas</NavLink>
