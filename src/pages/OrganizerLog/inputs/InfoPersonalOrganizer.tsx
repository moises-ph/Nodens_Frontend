...
const InfoPersonalorganizer = ({handlerInfo, alreadyOrganizer } : { handlerInfo : (name : string, lastname : string, genero : string, pais : string, ciudad : string, foto_perfil : string, telefono : string, fecha_nacimiento : string) => void, alreadyOrganizer : OrganizerT }) => {
  ...
  useEffect(() => {
    name.current!.value  = alreadyOrganizer.Name.length > 0 ? alreadyOrganizer.Name : "" ;
    Lastname.current!.value  = alreadyOrganizer.Lastname.length > 0 ? alreadyOrganizer.Lastname : "" ;
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} className='lg:w-1/2 w-full flex flex-col gap-5'>
        ...
        <button className='bg-orange-500 w-fit self-center px-4 py-1 rounded-lg text-white/80 hover:scale-105 transition' onClick={handleSubmit}>Listo</button>
      </form>
...
export default InfoPersonalorganizer;
