...
const CreateOffer = () => {
  ...
  const validations = (e:React.MouseEvent<HTMLInputElement, MouseEvent>) => {
    e.preventDefault();
    ...
    } else if(event_date.current!.value ==="") {
      Swal.fire({
        icon: 'warning',
        text: 'por favor ingresa una fecha',
        timer: 2500
      })
    } else if(payment.current!.value ==="") {
      Swal.fire({
        icon: 'warning',
        text: 'por favor un pago',
        timer: 2500
      })
    ...
    } else if(site_number.current!.value ==="") {
      Swal.fire({
        icon: 'warning',
        text: 'por favor ingresa un numero de ubicacion',
        timer: 2500
      })
    } else if(town.current!.value ==="") {
      Swal.fire({
        icon: 'warning',
        text: 'por favor ingresa un barrio',
        timer: 2500
      })
    } else if(vacants.current!.value ==="") {
      Swal.fire({
        icon: 'warning',
        text: 'por favor ingresa un numero de vacantes',
        timer: 2500
      })
    } else if(requeriments.length < 1) {
      Swal.fire({
        icon: 'warning',
        text: 'por favor ingresa al menos un requerimiento',
        timer: 2500
      })
    } else if(tags.length < 1) {
      Swal.fire({
        icon: 'warning',
        text: 'por favor ingresa al menos una tag',
        timer: 2500
      })
    } 
  ...
  return (
    <>
      <main className="w-full h-fit flex flex-col items-center justify-center gap-2 pt-10 bg-slate-100">
        <section className="md:w-2/5 w-full shadow-xl">
          <h1 className="text-2xl font-semibold text-slate-800 text-center">Crear Oferta</h1>
          ...
          <form className="w-full bg-slate-50 flex flex-col flex-wrap max-h-[50%] bg-transparent gap-3 px-10 pb-6 z-30" onSubmit={(e)=> e.preventDefault()}>
            <div className="w-full h-1/6 bg-transparent p-3 flex flex-col gap-6">
            ...
            <div className="w-full h-[20vh] bg-transparent p-3 flex md:flex-row flex-col justify-between gap-4">
            ...
            <div className="w-full h-fit bg-transparent p-3 flex md:flex-row flex-col gap-6">
              <div className="flex flex-col">
                ...
                <div className="flex border-solid border-b-[1px] border-slate-900">
                  ...
                  <input type="number" pattern="[0-9]+" min={0} name="Payment" onKeyPress={handleKeyPress} placeholder="Pago" ref={payment} className="font-thin  placeholder:font-thin outline-none" required/>
                </div>
              </div>
              <div className="flex flex-col">
                <label className="text-xl font-semibold text-gray-900">Vacantes: <span className="text-red-700">*</span></label>
                <input min={0} type="number" name="Vacants" placeholder="Vacantes" onKeyPress={handleKeyPress} ref={vacants} className="font-thin  border-solid border-b-[1px] border-slate-900 placeholder:font-thin outline-none" required/>
              </div>
            </div>
            
            <div className="w-full bg-transparent p-3 flex flex-col gap-4">
              <h5 className="text-xl font-semibold text-gray-900">Ubicacion del evento <span className="text-red-700">*</span></h5>
              <div className="flex md:flex-row flex-col w-full md:gap-2 gap-8">
                <div className="flex flex-col">
                  <label htmlFor="City" className="text-gray-900">Ciudad <span className="text-red-700">*</span></label>
                  <input type="text" name="City" placeholder="Ciudad" ref={city} className="font-thin h-2/5 border-solid border-b-[1px] border-slate-900 placeholder:font-thin md:w-32 outline-none" required/>
                </div>
                <div className="flex flex-col">
                  <label htmlFor="Town" className="text-gray-900">Barrio <span className="text-red-700">*</span></label>
                  <input type="text" name="Town" placeholder="Barrio" ref={town} className="font-thin h-2/5 border-solid border-b-[1px] border-slate-900 placeholder:font-thin md:w-32 outline-none" required/>
                </div>
                <div className="flex flex-col">
                  <label htmlFor="Career" className="text-gray-900">Carrera</label>
                  <input type="text" name="Career" placeholder="Carrera" ref={career} className="font-thin h-2/5 border-solid border-b-[1px] border-slate-900 placeholder:font-thin md:w-3/4 outline-none" />
                </div>
                <div className="flex flex-col">
                  <label htmlFor="Street" className="text-gray-900">Calle</label>
                  <input type="text" name="Street" placeholder="Calle" ref={street} className="font-thin h-2/5 border-solid border-b-[1px] border-slate-900 placeholder:font-thin md:w-3/4 outline-none" />
                </div>
                <div className="flex flex-col">
                  <label htmlFor="SiteNumber" className="text-gray-900">Numero <span className="text-red-700">*</span></label>
                  <input type="text" name="SiteNumber" placeholder="Numero" ref={site_number} onKeyPress={handleKeyPress}  className="font-thin h-2/5 border-solid border-b-[1px] border-slate-900 placeholder:font-thin md:w-3/4 outline-none" required/>
                </div>
              </div>
            </div>
            ...
            <div className="w-full bg-transparent p-3 flex flex-col gap-6">
            ...
            <input type="submit" onClick={(e)=> validations(e)} className="w-full h-10 bg-orange-500 text-slate-200 font-medium transition text-xl rounded-xl hover:shadow-2xl hover:shadow-black hover:ease-in hover:cursor-pointer hover:duration-200  active:translate-y-2 " value="Publicar"/>
          </form>
        </section>
      </main>
    </>
